...
function sendMessageToKotlin(messageType, data) {
  try {
    console.log("sending message to kotlin", messageType);
    SafeGazeInterface.sendMessageFromWebView(messageType, data);
  } catch {
...
const sendImgBackground = (imgInfo) => {
  console.log("sendImgBackground ", imgInfo)
...
async function runCommon() {
  ...
  updateVariable(isSiteFacebook2);
  console.log("==================", hostname);
  sendMessageToIOS("Script injection completedddddddddd", "dummy data");
//  settingsUpdate(settings);
  runObserving();
}
runCommon();

...
final class MoveNet: PoseEstimator {
  ...
  private let movenetMultiposeFile = FileInfo(name: "movenet_multipose_fp16", ext: "tflite")
  ...
  init(threadCount: Int, delegate: Delegates, modelType: ModelType) throws {
    ...
    try interpreter.resizeInput(at: 0, to: [1, 256, 256, 3])
  ...
  private func postprocess(imageSize: CGSize, modelOutput: Tensor) -> Person? {
    ...
    for idx in 0..<numKeyPoints {
      ...
      totalScoreSum += score
      if BodyPart.allCases.count > idx {
    ...
    return Person(keyPoints: keyPoints, score: totalScore, poseBox: self.cropRegion)
  }
  ...
  func estimateMultiplePoses(on pixelBuffer: CVPixelBuffer) throws -> [Person] {
    guard !isProcessing else {
        throw PoseEstimationError.modelBusy
    }
    isProcessing = true
    ...
    return postprocessMultiple(imageSize: pixelBuffer.size, modelOutput: outputTensor)
  }
    
  /// Convert normalized x and y coordinates ([0, 1]) to absolute coordinates in the input image.
  private func resizeKeypoint(x: CGFloat, y: CGFloat, imageWidth: CGFloat, imageHeight: CGFloat, cropRegion: RectF) -> CGPoint {
    return CGPoint(x: resizeX(x, imageWidth: imageWidth, cropRegion: cropRegion), y: resizeY(y, imageHeight: imageHeight, cropRegion: cropRegion))
  }

  private func resizeX(_ x: CGFloat, imageWidth: CGFloat, cropRegion: RectF) -> CGFloat {
    // Scale and translate the normalized x coordinate
    return x * (cropRegion.width * imageWidth) + cropRegion.left * imageWidth
  }

  private func resizeY(_ y: CGFloat, imageHeight: CGFloat, cropRegion: RectF) -> CGFloat {
    // Scale and translate the normalized y coordinate
    return y * (cropRegion.height * imageHeight) + cropRegion.top * imageHeight
  }

  private func postprocessMultiple(imageSize: CGSize, modelOutput: Tensor) -> [Person] {
    let output = modelOutput.data.toArray(type: Float32.self)
    let dimensions = modelOutput.shape.dimensions

    guard dimensions.count >= 3 else {
        fatalError("Unexpected tensor dimensions: \(dimensions)")
    }

    let numPersons = dimensions[1]
    let outputShapePerPerson = dimensions[2]

    // Constants
    let detectionThreshold: Float = 0.11
    let detectionScoreIndex = 55
    let boundingBoxYMinIndex = 51
    let boundingBoxXMinIndex = 52
    let boundingBoxYMaxIndex = 53
    let boundingBoxXMaxIndex = 54
    let keyPointStartIndex = 0
    let keyPointCount = 17
    let outputsPerKeyPoint = 3

    var persons: [Person] = []

    let inputWidth = CGFloat(inputTensor.shape.dimensions[1])
    let inputHeight = CGFloat(inputTensor.shape.dimensions[2])
    let widthRatio = imageSize.width / inputWidth
    let heightRatio = imageSize.height / inputHeight

    // Rescale all points to absolute coordinates
    for personIndex in 0..<numPersons {
        let baseIndex = personIndex * outputShapePerPerson
        let personScore = output[baseIndex + detectionScoreIndex]

        if personScore < detectionThreshold {
            continue
        }

        let yMin = output[baseIndex + boundingBoxYMinIndex]
        let xMin = output[baseIndex + boundingBoxXMinIndex]
        let yMax = output[baseIndex + boundingBoxYMaxIndex]
        let xMax = output[baseIndex + boundingBoxXMaxIndex]

        let boundingBox = RectF(
            left: CGFloat(xMin) * widthRatio,
            top: CGFloat(yMin) * heightRatio,
            right: CGFloat(xMax) * widthRatio,
            bottom: CGFloat(yMax) * heightRatio
        )

        var keyPoints: [KeyPoint] = []
        for keyPointIndex in 0..<keyPointCount {
            let keyPointBaseIndex = baseIndex + keyPointStartIndex + keyPointIndex * outputsPerKeyPoint
            let normalizedY = CGFloat(output[keyPointBaseIndex])
            let normalizedX = CGFloat(output[keyPointBaseIndex + 1])
            let score = output[keyPointBaseIndex + 2]

            // Resize keypoint to absolute coordinates
            let absolutePoint = resizeKeypoint(
                x: normalizedX,
                y: normalizedY,
                imageWidth: imageSize.width,
                imageHeight: imageSize.height,
                cropRegion: cropRegion ?? RectF(left: 0, top: 0, right: 1, bottom: 1) // Default full image if cropRegion is nil
            )

            let keyPoint = KeyPoint(
                bodyPart: BodyPart.allCases[keyPointIndex],
                coordinate: absolutePoint,
                score: score
            )
            keyPoints.append(keyPoint)
        }

        let person = Person(keyPoints: keyPoints, score: personScore, poseBox: boundingBox)

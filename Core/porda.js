...
const loadVideoElement = async (video) => {
    ...
    if (video == null ? void 0 : video.isLoaded) return true;
...
const replaceImgSrc = (imageNode, newSrc) => {
    ...
    Array.from(imageNode.attributes).forEach((attr) => {
        if (
            attr.name === "srcset" ||
            attr.name === "src" ||
            srcsetAttributesRegex.test(attr.name) ||
            srcAttributesRegex.test(attr.name)
        ) {
...
const observeElement = (node) => {
    ...
    if (node.tagName === "IMG") {
        if (
            ((_a = node.dataset) == null ? void 0 : _a.UnderProcessing) ===
            "true"
        ) {
            return;
        }
        node.dataset.PordaCondition = "CheckingByMutationObserver";
        const nodeSrc = node.hasAttribute("src")
            ? node.src
            : node.getAttribute("srcset");
        if (!(node.dataset.PordaOriginalSrc === nodeSrc)) {
            ...
            node.dataset.PordaCondition =
                "CheckingByMutationObserver-NodeSrcUpdateds";
        }
        observeImgNode$1(node);
        return;
    }
...
const observeImgNode$1 = async (img2) => {
    if (img2.dataset.pordaDone) return;
    if (img2.dataset.PordaResult) return;
    ...
    let imgSrc = img2.hasAttribute("src")
        ? img2.src
        : img2.getAttribute("srcset");
    if (!imgSrc) {
...
const observeVideoNode = (videoNode) => {
    var _a, _b;
    if (videoNode.clientHeight < 200) return;
    if (videoNode.clientHeight < 200) return;
    if (videoNode.clientWidth < 150) return;
    if (videoNode.clientWidth < 300 && videoNode.clientHeight < 200) return;
    if (
        !videoNode.dataset.PordaCondition &&
        (((_a = videoNode.src) == null ? void 0 : _a.length) > 0 ||
            ((_b = videoNode.getElementsByTagName("source")) == null
                ? void 0
                : _b.length) > 2)
    ) {
...
const intervalChecking = async () => {
    const images = document.querySelectorAll(
        'img[data--porda-condition="request-reobserve"], img:not([data--porda-condition])'
    );
    images.forEach((img2) => {
        if (img2.dataset.PordaCondition) {
            if (img2.dataset.PordaCondition === "request-reobserve") {
                img2.dataset.PordaCondition = "request-reobserving";
                observeImgNode$1(img2);
            }
        }
        if (!img2.dataset.PordaCondition) {
            observeImgNode$1(img2);
